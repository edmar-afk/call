...
function initPeer(roomId) {
	...
	peer.on("open", (peer_id) => {
		console.log("(This is from initPeer Function) Peer Connected with ID: " + peer_id);
...
function joinRoom() {
	peer = new Peer();
	peer.on("open", (join_id) => {
		console.log("(This is from joinRoom function) Connected with Id: " + join_id);
		getUserMedia(
			{ video: true, audio: true },
			(stream) => {
				local_stream = stream;
				setLocalStream(local_stream);
				success("Joining peer");
				let call = peer.call(join_id, stream);
...
function handleIncomingCall(call) {
	if (confirm("Accept this call?") == true) {
...
const toggleCamera = async () => {
	...
	try {
		if (isCameraOn) {
			...
			// Turn on the camera
			localSteam = await navigator.mediaDevices.getUserMedia({ video: true });
